...
このプロジェクトは Python + uv で環境を構築する。

前提:

- Git が使えること
- uv が使えること
- Python 3.12 系を使用すること
- RiceSEGデータセットと学習済みcheckpointはGit管理しないため、別途配置すること
...
`uv sync` でうまく入らない場合は、最低限以下を追加する。

```bash
uv add torch torchvision opencv-python numpy matplotlib pillow tqdm ultralytics
```
...
#### 4. RiceSEGデータセットを配置する

RiceSEGはGit管理しないため、以下の位置に配置する。
...
```bash
uv run python riceseg_dataset.py
```
...
学習済みモデルはGit管理しないため、以下に配置する。

```text
alcon2026/
└── checkpoints/
    └── riceseg_unet_alcon3_global_w45_best.pth
```
...
Windowsでは PowerShell を使用する。

#### 1. Git と uv を用意する

Git for Windowsをインストールする。

uvはPowerShellで以下を実行してインストールする。
...
```powershell
cd C:\Users\<ユーザー名>\Desktop\alcon2026
git pull
```
...
```bash
uv run python riceseg_dataset.py
uv run python train_riceseg_unet.py
uv run python main.py
```

現在参照しているcheckpointは、`riceseg_segmenter.py` の以下で確認する。

```python
CHECKPOINT_PATH = Path("checkpoints/riceseg_unet_alcon3_global_w45_best.pth")
```
